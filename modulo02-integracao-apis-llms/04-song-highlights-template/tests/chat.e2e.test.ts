...
describe('Chat de Recomendação Musical - Testes E2E', () => {
  ...
  it('Deve sumarizar o histórico da conversa', async () => {
    const testThreadId = `test-user-${Date.now()}`;
    const config = {
      configurable: { thread_id: testThreadId },
      context: { userId: testThreadId }
    ...
    await graph.invoke(
      {
        messages: [new HumanMessage('Oi, sou Taylor e adoro música pop'),
          new HumanMessage('mas gosto de rock às vezes'),
          new HumanMessage('Detesto funk, não é meu estilo'),
          new HumanMessage('Não suporto música muito lenta, prefiro algo animado'),
          new HumanMessage('Oi, sou Taylor e adoro música pop'),
          new HumanMessage('Oi, sou Taylor e adoro música pop'),
          new HumanMessage('Adoro a banda Metalica!'),
        ],
        userId: testThreadId,
      },
      config
